...
class SoftMaxCrossEntropy:
    ...
    def forward(self, z: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        return self._softmax(z), self._cross_entropy(y,self._softmax(z))
...
class Linear:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
       #should be same shape as weights
        self.dw = np.outer(dz, self.inputs)
        return np.dot(dz, self.w[:, 1:])
        

    def step(self, isit) -> None:
        """
        Apply SGD update to weights using self.dw, which should have been 
        set in NN.backward().
        """
        # TODO: implement
        #updaten with gradient
...
class NN:
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        a1 = self.sigmoid.forward(z1)
        z2 = self.linear2.forward(a1)
        return SoftMaxCrossEntropy().forward(z2, y)
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        #hiddenlayer/output
        gradient_z2 = SoftMaxCrossEntropy().backward(y, y_hat)
        gradient_a1 = self.linear2.backward(gradient_z2)
        #firsylayer/hidden
        gradient_z1 = self.sigmoid.backward(gradient_a1)
        self.linear1.backward(gradient_z1)
        #
    ...
    def step(self):
        ...
        #firsylayer/hidden
        self.linear1.step(True)
        #hiddenlayer/output
        self.linear2.step(False)
    ...
    def compute_loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        for i in range(len(X)):
            loss = self.forward(X[i], y[i])[1]
            avgloss = avgloss+ loss
        return avgloss/len(X)
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for i in range(n_epochs):
            ...
            shufx, shufy = shuffle(X_tr, y_tr, i)
            for x, y in zip(shufx, shufy):
                #calculate output 
                y_hat= self.forward(x, y)[0]
                #backpropagation
                self.backward(y, y_hat)
                #update weights w gradients
                self.step()

            trainloss = self.compute_loss(shufx, shufy)
            testloss = self.compute_loss(X_test, y_test)
            train_losses= np.append(train_losses, trainloss)
            test_losses= np.append(test_losses, testloss)
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        for x, y_true in zip(X, y):
            y_hat=self.forward(x, y_true)[0]
            #gets first index of max
            predict = np.argmax(y_hat)
            if predict != y_true:
                wrong =wrong+ 1
            labels = np.append(labels, predict)
